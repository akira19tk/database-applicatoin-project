...
export default function InvoiceForm({ onSubmit, submitting, initialData }) {
  ...
  // Fetch vat_percent from config on create mode
  React.useEffect(() => {
    if (!initialData) {
      getConfig("vat_percent")
        .then((val) => setVatPercent(Number(val)))
        .catch(() => {});
    }
  }, []);
  ...
  React.useEffect(() => {
    if (initialData) {
      ...
      setInvoiceDate(d);
      setVatPercent(Math.round(Number(initialData.vat_rate || 0.07) * 100));
  ...
  const totalDiscount = items.reduce((s, it) => {
    ...
    return s + Math.round(extended * pct / 100 * 100) / 100;
  }, 0);
  ...
  function handleSubmit(e) {
    ...
    const payload = {
      ...
      vat_rate: Number(vatPercent) / 100,
      ...
      <div className="invoice-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 16, marginBottom: 16 }}>
        ...
        <div className="card invoice-summary-card" style={{ height: "fit-content" }}>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.875rem", color: "var(--text-muted)" }}>
              <span>VAT ({Number(vatPercent).toFixed(0)}%)</span>
