        ...
        if (mode === "edit") {
          ...
          setLines(
            (data.line_items || []).map((li) => ({  
              ...
              amount_remain: Number(li.amount_remain_before_this ?? li.amount_remain ?? 0),   // amount_remain_before_this
              amount_received: Number(li.amount_received || 0),   // amount_received
            })),
          );
        }
        setLoading(false);
  ...
  function handleInvoiceSelect(row) {
    setLines((prev) => {
      ...
      next[invoicePickerLineIdx] = {
        ...
        amount_received: Number(row.amount_remain || 0),   // amount_remain
      };
      return next;
    });
  }
