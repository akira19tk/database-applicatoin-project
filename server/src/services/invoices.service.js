...
export async function getInvoice(idOrInvoiceNo) {
  ...
  const header = await pool.query(
    `
      select i.invoice_no, i.invoice_date, i.customer_id, i.total_amount, i.vat, i.amount_due,
       i.vat_rate, i.total_price, i.total_discount, i.vat_amount,
...
/** Resolve product_code to id and compute discount fields. */
    ...
    const inv = await client.query(
      `
        insert into invoice (id, created_at, invoice_no, invoice_date, customer_id, sales_person_id,
          total_amount, vat, amount_due, vat_rate, total_price, total_discount, vat_amount)
        values (
      ...
      [resolvedInvoiceNo, invoice_date, customer_id, sales_person_id,
       total_price, vat_amount, amount_due, Number(vat_rate), total_price, total_discount, vat_amount],
    );
    ...
    await client.query(
      `
        ...
        SET invoice_no=$1, invoice_date=$2, customer_id=$3, sales_person_id=$4,
          ...
          vat_rate=$8, total_price=$9, total_discount=$10, vat_amount=$11
        WHERE id=$12
      `,
      [resolvedInvoiceNo, invoice_date, customer_id, sales_person_id,
       total_price, vat_amount, amount_due, Number(vat_rate), total_price, total_discount, vat_amount, id],
    );
      ...
      if (lineId) {
        await client.query(
          ...
          [li.product_id, li.quantity, li.unit_price, li.extended_price,
           li.line_discount_percent, li.line_discount_amount, li.line_net_price, lineId, id],
        );
      } else {
        await client.query(
          `
